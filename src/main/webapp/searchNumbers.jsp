...
<%
    ...
    if (startWithNumber == null) {
        startWithNumber = new String[]{"9", "8", "7", "6"};
    }
    if (doesNotContain == null) {
        doesNotContain = new String[]{"x"};
    }
    ...
    if (pricefilter == null) {
        request.getSession(false).setAttribute("doesNotContain", doesNotContain);
        request.getSession(false).setAttribute("startWith", startWithNumber);
        request.getSession(false).setAttribute("isSearchApply", isSearchApply);
        request.getSession(false).setAttribute("priceRange", priceRange);
    }
    if (vanityCheck != null) {
    ...
    if (vanityCheck != null || isSearchApply != null) {
        ...
        if ("low".equalsIgnoreCase(pricefilter) || "high".equalsIgnoreCase(pricefilter)) {
            startWithNumber = (String[]) request.getSession(false).getAttribute("startWith");
            doesNotContain = (String[]) request.getSession(false).getAttribute("doesNotContain");
            isSearchApply = (String) request.getSession(false).getAttribute("isSearchApply");
            priceRange = (String) request.getSession(false).getAttribute("priceRange");
        }
        String orderby = "";
        if (saerchCriteria != null && saerchCriteria.equalsIgnoreCase("notEvenNumber") && pricefilter != null) {
            products = generfunc.getProductListWithout248(pricefilter);
        } else if (isSearchApply != null) {
            orderby = "";
            if (pricefilter == null) {
                products = generfunc.productListSearchFilter(priceRange, startWithNumber, doesNotContain);
            ...
            if (pricefilter == null) {
                ...
                orderby = "desc";
                products = generfunc.getProductFilterList(orderby);
            }
        }
                                    ...
                                    <ul>
                                        <li>
                                            <form action="searchNumbers" method="post">
                                                ...
                                                <div class="col-sm-12"
                                                     style="padding: 1px; display: flex; margin-bottom: 2px">
                                                    <div style="width: 85%">
                                                        <button type="submit" class="btn btn-primary searchicon">
                                                            Without 2, 4, 8
                                                        </button>
                                                    </div>
                                                    <div style="margin-left: 5%;"><img src="assets/img/favorites.png"/>
                                                    </div>
                                                </div>
                                            </form>
                                        </li>
            ...
            <div class="col-md-9 col-sm-12">
                    ...
                    <aside class="blog-search" style="margin-bottom:0px;">
                        <form action="searchNumbers" method="post" id="searchNumbersFormId">
                            <div class="col-sm-3" style="padding: 1px">
                                <select name="saerchCriteria" id="saerchCriteriaId"
                                        style="width: 100%; height: 50px; border: 1px solid #e1e1df; font-family: 'Roboto', sans-serif; outline: 0;">
                            ...
                            <div class="col-sm-9" style="padding: 1px">
                                <input name="search" type="text" placeholder="Search Number" style="height:50px"
                                       id="searchNumbersSearchId">
                            </div>
                            <div class="col-sm-0" style="padding: 1px">
                                <button type="submit" class="search-btn searchicon" style=""><i
                ...
                <div class="row">
                    <nav class="sortingOptions button-group mini clearfix" style="margin-bottom:20px;">
                        ...
                        <a href="searchNumbers?search=<%=productName%>&pricefilter=low&saerchCriteria=<%=saerchCriteria%>"
                           class="btn btn-default">Price Low to
                            High</a>
                        <a href="searchNumbers?search=<%=productName%>&pricefilter=high&saerchCriteria=<%=saerchCriteria%>"
                           class="btn btn-default">Price High
                            to Low</a>
                        <a href="searchNumbers?search=<%=productName%>&pricefilter=sortNumbers&saerchCriteria=<%=saerchCriteria%>"
                ...
                <div class="overflowhi">
                    ...
                    %>
                    <br/><br/>
                    <center><h2 class="heading_for_subtitle">Oops Number Not Found... </h2></center>
                    <%} %>
...
<%
        ...
        System.out.println("Error in Search  :- " + new Date() + " >>>  " + vanityCheck);
        String remoteAddr = "";
        if (request != null) {
