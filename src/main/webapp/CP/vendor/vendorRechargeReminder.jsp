...
<head>
  ...
  <title>VIP NUMBER STORE | Vendor</title>
...
<div class="wrapper">
  ...
  <header class="main-header">
	...
	if(id==null){response.sendRedirect("vendorLogin");
  			}else{
		%>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Recharge Details</li>
      </ol>
    </section>
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <%
                                		...
                                		ResultSet res = stmt.executeQuery("select * from ProductDetails where vendor_id ='"+id+"' and recharge_validity!='' and sizelength!='' order by sizelength");
                                        int sr=0;
                                        String rechargeDate="";
                                        int rechargeValidity=0;
                                        Date recharge_Date = null, nextRecharge=null;
                                		...
                                		while(res.next()) {
                                			sr++;
                                			rechargeDate = res.getString("sizelength");
                                			recharge_Date = formatter.parse(rechargeDate);
                                			rechargeValidity = res.getInt("recharge_validity");
                                			c.setTime(formatter.parse(rechargeDate));
                                			c.add(Calendar.DAY_OF_MONTH, rechargeValidity);  
                                			
                                			nextRecharge = c.getTime();
                                			
                                			
										%>
                                        <tr>
                                        <td class="center"><%=sr %></td>
                                           <td class="center"><%=res.getString("number") %></td>
                                            <td class="center"><%=formatter1.format(recharge_Date)%></td>
                                            <td class="center"><%=rechargeValidity%></td>
                                            <td class="center"><%=formatter1.format(nextRecharge)%></td>
