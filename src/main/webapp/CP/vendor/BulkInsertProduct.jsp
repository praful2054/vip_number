		...
		try {
			...
			if (multi != null) {
				...
				while((str = br.readLine()) != null) {
					...
					product.setProductName(values[1]==null?"":values[1]);
					product.setNumber(values[1].replaceAll("\\s+","").trim());
					product.setType("None");
					product.setProductCode(values[2]==null?"":values[2]);
					product.setProductBrand(values[3]==null?"":values[3]);
					
					product.setPacking(values[4]==null?"":values[4]);
					product.setVegProduct(values[5]==null?"":values[5]);
					product.setBuyerName(values[6]==null?"":values[6]);
					product.setBuyerContact(values[7]==null?"":values[7]);
					product.setCurrentNumber("");
					Random rand = new Random();
					product.setFileID(Integer.toString(rand.nextInt()));
					
					
					
					if(values[1]!=null){
					String prdtName = values[1].replaceAll("\\<[^>]*>","");
			...
			int sum = generfunc.sumNumber(prdtName.replaceAll("\\s+","").trim());
					...
					productOtherDetail.setRateblack(Integer.parseInt(values[8]));
					productOtherDetail.setDiscount(Integer.parseInt(values[9]));
