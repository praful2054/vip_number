  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Offline Enquiry</li>
      </ol>
    </section>
	...
	<div class="box box-success">
	 ...
	 ResultSet rs =null;PreparedStatement prepare1=null,prepare3=null;
	 try{
		
		
		 con=new DBConnection().getCon();
         int generatedKey=0;
		 prepare1=con.prepareStatement("select id from customerEnquiry where Mobile_NO='"+request.getParameter("cmobile")+"'");
		 rs = prepare1.executeQuery();
		 if(rs.next()){
			 generatedKey =  rs.getInt(1);
		 }
		 if(generatedKey==0){
			 String insert ="insert into customerEnquiry values(0, ?, ?,?,?,?,?,Now(),?,?,Now(),?)";
			    prepare=con.prepareStatement(insert,Statement.RETURN_GENERATED_KEYS);
				prepare.setString(1,request.getParameter("cname"));
				prepare.setString(2,request.getParameter("cemail"));
				prepare.setString(3,request.getParameter("cmobile"));
				prepare.setString(4,request.getParameter("ccity"));
				prepare.setString(5,"");
				prepare.setString(6,"");
				prepare.setString(7,"New");
				prepare.setString(8,"");
				prepare.setString(9,"Offline");
				int row=prepare.executeUpdate();
			
				ResultSet res = prepare.getGeneratedKeys();
				
				if (res.next()) {
				    generatedKey = res.getInt(1);
				}
		 }else{
				PreparedStatement pst3 = con.prepareStatement("update  customerEnquiry set updatedDate=Now(),status='New' where id="+generatedKey+"");
				pst3.executeUpdate();
			}
		 
		    prepare3=con.prepareStatement("insert into customerEnquiryDetails values(0, ?, ?, ?, ?, ?, Now(), ?)");
			prepare3.setString(1,String.valueOf(generatedKey));
			prepare3.setString(2,request.getParameter("etype"));
			prepare3.setString(3,request.getParameter("requirement"));
			prepare3.setString(4,request.getParameter("budget"));
			prepare3.setString(5,request.getParameter("esource"));
			prepare3.setString(6,eid);
