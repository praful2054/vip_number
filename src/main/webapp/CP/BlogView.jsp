            ...
            <div class="box-body">
              <table id="example1" class="table table-striped table-hover dt-responsive" width="100%">
                                    ...
                                    <tbody>
                                		...
                                		ResultSet res = stmt.executeQuery("select * from Blog");
                                        while(res.next()) {
										%>
                                        <tr>
                                        <td class="center"><%=res.getString("id") %></td>
                                            <td class="center"><%=res.getString("blogTitle") %></td>
                                            <%if(res.getString("uploadType").equals("Image")) {%>
                                             <td class="center">Image</td>
                                             <%} else if(res.getString("uploadType").equals("Video")) {%>
                                             <td class="center">Video</td>
                                             <%} else{ %>
                                             <td class="center">None</td>
                                             <%} %>
                                            <td class="center"><a href="blogComments?id=<%=res.getString("id") %>">Comments</a></td>
                                            <td class="center">
												
												<a class="glyphicon glyphicon-trash" href="DeleteBlog.jsp?id=<%=res.getString("id") %>&image=<%= res.getString("img_loc")%>"></a>
											</td>
