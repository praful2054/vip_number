            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <%
                                		...
                                		String name="";
                                		ResultSet res = stmt.executeQuery("select * from vendorDetails where status='"+Status+"'");
                                        while(res.next()) {
                                        ...
                                        <td class="center"><%=res.getString("id") %></td>
                                           <td class="center"><%=res.getString("loginId") %></td>
                                           <td class="center"><%=res.getString("password") %></td>
                                            <td class="center"><%=name%> </td>
                                            <td class="center"><%=res.getString("contactno1") %></td>
											<td class="center"><%=res.getString("isRegistered")==null?"":res.getString("isRegistered") %></td>
											 <td class="center">
													<%
														if(res.getString("status").equalsIgnoreCase("Y")){
															%>
																<a href="activate?id=<%= res.getString("id")%>&activate=N&type=vendor" onclick="return confirm('Are you sure you want to Deactivate this Vendor ?');">Deactivate</a>
															<%	
														}
														else{
															%>
																<a href="activate?id=<%= res.getString("id")%>&activate=Y&type=vendor" onclick="return confirm('Are you sure you want to activate this Vendor ?');">Activate</a>
															<%
														}
													%>  
													
                                            <td class="center">
                                                    <a class="glyphicon glyphicon-edit" href="vendorEditForm?id=<%= res.getString("id")%>"></a>&nbsp;&nbsp;
													<a class="glyphicon glyphicon-trash" href="delete?id=<%=res.getString("id") %>&type=vendor&status=<%=Status%>" onclick="return confirm('Are you sure you want to delete this Vendor?');"></a>
												</td>
