  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Category Edit Form</li>
      </ol>
    </section>
	 ...
	 <div class="box-body">
	                   <%
						       ...
						       String parent = request.getParameter("parent");
							    ...
							    Category categories = dbAction.getSingleData(QueryConstants.CATEGORY_SINGLE_DATE_QUERY, Category.class, Integer.parseInt(id));
						%>
        <form class="form-horizontal" action="updateCategories"  method="post" name="form" ENCTYPE="multipart/form-data">
                                   
                                    <%
										if(categories.getCate_parent_id()!=0){
											
											GeneralFunction generfunc = (GeneralFunction) context.getBean("generalFunction");
											//List<Category> parentCategoriesList = generfunc.getParentCategoryList(parent);
									...
									%>
                                       <div class="form-group">
                                            ...
                                            <div class="col-sm-6">
                                                <select  style="width:250px" name="parentCategoryId">
                                                    <%
														for(Category category : parentCategoryList){
															%>
																<option value="<%=category.getCate_id() %>" <%if(categories.getCate_parent_id() == category.getCate_id() ){ %>selected<%} %>><%=category.getCate_name() %></option>
															<%
														}
													%>
                                                </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-6">
                                                <input class="form-control" id="focusedInput" name="cate_name" type="text" value="<%= categories.getCate_name() %>" required="required">
                                            </div>
                                        </div>
										 ...
										 <div class="form-group">
                                           ...
                                           <div class="col-sm-10">
                                                <textarea id="editor1" name="cate_descript" rows="10" cols="80"><%= categories.getCate_descript() %></textarea>
                                            </div>
                                        </div>
                                        
                                        <%
											if(categories.getCate_img_loc() != null && !categories.getCate_img_loc().equalsIgnoreCase("null")) {
										%>

										<div class="form-group">
                                            <label class="col-sm-2 control-label">Image Name</label>
                                            <div class="col-sm-10">
                                               <img src="<%=application.getRealPath("/CategoriesImage") +"/"+ categories.getCate_img_loc() %>">
                                            </div>
                                        </div>
									...
									<%
                                        ...
                                        <div class="box-footer">
                                            <button type="submit" class="btn btn-info pull-right">Update Category Details</button>
                                        </div>
                                        <input type="hidden" name="hdnId" id="hdnId" value="<%=categories.getCate_id() %>">
									<input type="hidden" name="hdncate_img_upload" id="hdncate_img_upload" value="<%=categories.getCate_img_loc() %>">
                                </form>
