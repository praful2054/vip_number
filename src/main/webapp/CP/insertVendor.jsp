	...
	<div class="box box-success">
	 ...
	 try{
		 ...
		 PreparedStatement prepare = null;
		   ...
		   if (multi!=null){
			PreparedStatement prepare1=con.prepareStatement("select loginId from vendorDetails where loginId='"+multi.getParameter("email")+"'");
			ResultSet rs = prepare1.executeQuery();
			if(rs.next()){
			%>
			<div class="alert alert-error">
         <button type="button" class="close" data-dismiss="alert">×</button>
         <strong>Vendor Details Alredy Exits....</strong>
        </div>
			
			<%
			}else{
				prepare=con.prepareStatement("insert into vendorDetails values(0, ?, ?, ?, ?, ?, ?, ?, ? ,curDate(),? ,?, ?, ?, ?, ?, ?, ?, ?, ?)");
				prepare.setString(1,multi.getParameter("email"));
				prepare.setString(2,multi.getParameter("password"));
				prepare.setString(3,multi.getParameter("name"));
				prepare.setString(4,multi.getParameter("address"));
				prepare.setString(5,multi.getParameter("address2"));
				prepare.setString(6,multi.getParameter("tel"));
				prepare.setString(7,multi.getParameter("website"));
				prepare.setString(8,"N");
				prepare.setString(9,multi.getParameter("paytmNo"));
				prepare.setString(10,multi.getParameter("GPayNO"));
				prepare.setString(11,multi.getParameter("isregister"));
				prepare.setString(12,multi.getFilesystemName("cate_img_upload"));
				prepare.setString(13,multi.getParameter("nickName"));
				prepare.setString(14,multi.getParameter("bankName"));
				prepare.setString(15,multi.getParameter("accountName"));
				prepare.setString(16,multi.getParameter("accountNumber"));
				prepare.setString(17,multi.getParameter("branchLocation"));
				prepare.setString(18,multi.getParameter("ifscCode"));
				int row=prepare.executeUpdate();
	 %>
		 <div class="callout callout-success">
                <h4>success callout!</h4>

                <p>Vendor Add Sucess........</p>
              </div>
		 <%}}}
