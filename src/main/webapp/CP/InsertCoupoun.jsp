...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	<div class="box box-success">
	 ...
	 <%
	 try{
		 String odate = request.getParameter("date")==null?"":request.getParameter("date");

         if (odate != null && !odate.equalsIgnoreCase("")) {

             odate = request.getParameter("date").substring(6, 10) + "-" + request.getParameter("date").substring(0, 2) + "-" + request.getParameter("date").substring(3, 5);

         } else {

             odate = new SimpleDateFormat("yyyy-MM-dd").format(Calendar.getInstance().getTime());

         }
		PreparedStatement prepare = null;
		Connection con=new DBConnection().getCon();
		PreparedStatement prepare1=con.prepareStatement("select coupoun_code, coupoun_type from Coupoun where coupoun_code='"+request.getParameter("coupounName")+"' and coupoun_type='"+request.getParameter("coupounType")+"' and expiry_date > curDate()");
		ResultSet rs = prepare1.executeQuery();
		if(rs.next()){
			%>
			<div class="alert alert-error">
         <button type="button" class="close" data-dismiss="alert">×</button>
         <strong>Coupoun Code Alredy Exits....</strong>
        </div>
			
			<%
		}else{
			 prepare=con.prepareStatement("insert into Coupoun values(0, ?, ?, ?, ?, ?, curDate(), ?, ?, ? ,curDate())");
			prepare.setString(1,request.getParameter("coupounName"));
			prepare.setString(2,request.getParameter("coupounType"));
			prepare.setString(3,request.getParameter("user"));
			prepare.setString(4,request.getParameter("userid"));
			prepare.setString(5,request.getParameter("CoupounValue"));
			prepare.setString(6,odate);
			prepare.setString(7,"0");
			prepare.setString(8,request.getParameter("Y"));
