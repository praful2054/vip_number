...
try{
	...
	Connection con=new DBConnection().getCon();
		...
		prepare=con.prepareStatement("insert into offlineenquiryfollowup values(0, ?, ?, ?,Now())");
		...
		if(profile.equalsIgnoreCase("Admin") || profile.equalsIgnoreCase("Lead")|| profile.equalsIgnoreCase("SEO") || profile.equalsIgnoreCase("Data Entry")){
			prepare1=con.prepareStatement("update offlineenquiry set status=?,EmployeeId=? where id=? ");
		...
		prepare1=con.prepareStatement("update offlineenquiry set status=? where id=? ");
		prepare1.setString(1,request.getParameter("status"));
		...
		response.sendRedirect("offlineEnquiresFollowup?id="+request.getParameter("enq_id"));
