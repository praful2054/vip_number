...
try{
	...
	Connection con=new DBConnection().getCon();
		...
		prepare=con.prepareStatement("insert into ivrDemofollowup values(0, ?, ?, ?,Now())");
		...
		if(profile.equalsIgnoreCase("Admin") || profile.equalsIgnoreCase("Lead") || profile.equalsIgnoreCase("SEO")){
			prepare1=con.prepareStatement("update scheduleDemo set status=?,EmployeeId=? where id=? ");
		...
		prepare1=con.prepareStatement("update scheduleDemo set status=? where id=? ");
		prepare1.setString(1,request.getParameter("status"));
		...
		response.sendRedirect("ivrDemoFollowup?id="+request.getParameter("enq_id"));
