              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <%
			                ...
			                SimpleDateFormat inputFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						                ...
						                ResultSet res = stmt.executeQuery("select cl.cust_id,cl.emailid,cl.password,cl.createdDate, cb.firstName,cb.contactNo1,cb.state,cb.city,cb.pincode,cl.status from CustomerLoginDetails cl, CustomerBillingDetails cb where cl.cust_id= cb.cust_id order by cl.createdDate desc ");
                                        while (res.next()){
                                            ...
                                            <td class="center">
                                                <%
													if(res.getString("status").equalsIgnoreCase("Y")){
														%>
															<a href="activate?id=<%= res.getString("cust_id")%>&activate=N&type=customer">Deactivate</a>
														<%	
													}
													else{
														%>
															<a href="activate?id=<%= res.getString("cust_id")%>&activate=Y&type=customer">Activate</a>
														<%
													}
												%>
                                            </td> 
                                            <td class="center">
                                            		<a class="glyphicon glyphicon-edit" href="editCustomer?id=<%= res.getString("cust_id")%>"></a>&nbsp;&nbsp;
												<%if(profile.equalsIgnoreCase("Admin")){ %>	
													<a class="glyphicon glyphicon-trash" href="delete?id=<%= res.getString("cust_id")%>&type=customer"></a>
												<%} %>
												</td>
