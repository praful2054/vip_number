...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Offline Enquires   &nbsp;&nbsp;&nbsp;&nbsp;  <a href="offlineEnquires">Add Offline Enquires</a>
      </h1>
      
     
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Offline  Enquires Details</li>
      </ol>
    </section>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss");
                                		...
                                		if(profile.equalsIgnoreCase("Admin") || profile.equalsIgnoreCase("Lead") || profile.equalsIgnoreCase("SEO") || profile.equalsIgnoreCase("Data Entry")){
                                			sql = "select * from offlineenquiry order by id desc";
                                		}else{
                                			sql = "select * from offlineenquiry where EmployeeId ='"+id+"'  order by id desc";
                                		}
                                		...
                                		while(res.next()) {
                                        	...
                                        	if(res.getString("createdBy")==null || !res.getString("createdBy").contains("@")){
												updatedby = res.getString("createdBy")==null?"NA":res.getString("createdBy");
											}else{
												int index = res.getString("createdBy").indexOf('@');
												//e//mail = email.substring(0,index);
												updatedby = res.getString("createdBy").substring(0,index);
											}
                                        ...
                                        <td class="center"><%=sr %></td>
                                         <td class="center"><%=res.getString("type")==null?"":res.getString("type") %></td>
                                           <td class="center"><%=res.getString("requirement") %></td>
                                           <td class="center"><%=res.getString("budget") %></td>
                                            <td class="center"><%=res.getString("Customer_Name") %></td>
                                            <td class="center"><%=res.getString("Customer_Mobile") %></td>
                                           
                                            <td class="center"><%=res.getString("Enquiry_Source") %></td>
											<td class="center"><%=res.getString("status") %></td>
											<td class="center"><%=res.getString("UpdatedDate")%> <%if(updatedby!=null && !updatedby.equals("NA")){%><font size="1"> by <%=updatedby%><%} %></font></td>
											 <td class="center"><%=Full_Name %></td>
													
                                            <td class="center">
                                                    <a class="glyphicon glyphicon-edit" href="offlineEnquiresFollowup?id=<%= res.getString("id")%>"></a>&nbsp;&nbsp;
											</td>
