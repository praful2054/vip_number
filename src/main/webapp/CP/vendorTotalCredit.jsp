...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
            ...
            <!-- /.box-header -->
            <div class="box-body">
            <form name="search" id="search" method="post" action="vendorTotalCredit">
              <table id="example1" class="table table-bordered table-striped">
                ...
                SimpleDateFormat inputFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                		...
                                		if(startDate !=null && endDate!=null && !startDate.equals("null") && !endDate.equals("null")){
                                		 res = stmt.executeQuery("select po.*, pd.number,ot.orderId, ot.orderPlacedDate  from OrderPurchaseDetails op, OrderTransaction ot, ProductDetails pd, ProductOtherDetails po where pd.vendor_id='"+VendorId+"' and op.product_id = pd.product_id and ot.orderId = op.orderId  and pd.product_id = po.product_id and ot.paymentStatus='Y' and ot.orderStatus='Order Confirmed' and DATE(ot.orderPlacedDate) between '"+startDate+"' and '"+endDate+"' ");
                                		}else{
                                			 res = stmt.executeQuery("select po.*, pd.number, ot.orderId, ot.orderPlacedDate from OrderPurchaseDetails op, OrderTransaction ot, ProductDetails pd, ProductOtherDetails po where pd.vendor_id='"+VendorId+"' and op.product_id = pd.product_id and ot.orderId = op.orderId  and pd.product_id = po.product_id and ot.paymentStatus='Y' and ot.orderStatus='Order Confirmed'");
                                		}
                                		while(res.next()) {
                                        	...
                                        	rateblack=0;
                                            	...
                                            	rateblack = res.getInt("rateblack");
                                            	productprice = res.getString("rateInRupee");
                                        	
										%>
										<tr>
                                            <td class="center"><%=sr %></td>
                                            <td class="center">VN000<%=res.getString("orderId") %> </td>
                                            <td class="center"><%=outputFormat.format(inputFormat.parse(res.getString("orderPlacedDate"))) %> </td>
                                            <td class="center"><%=Number %></td>
                                            <td class="center"><%=rateblack%></td>
              ...
              <input type="hidden" name="vendorId" id="vendorId" value="<%=request.getParameter("vendorId")%>" />
    ...
    $('#daterange-btn').daterangepicker(
      {
        ranges   : {
          ...
          'Last 9 Days' : [moment().subtract(8, 'days'), moment()],
