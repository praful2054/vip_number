               ...
               <%
						               ...
						               SimpleDateFormat inputFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                        ...
                                        if(profile.equalsIgnoreCase("Sales")){
                                        	...
                                        	orderTransactions = dbAction.getDataList(QueryConstants.ORDER_TRANSACTION_ALL_Confirm_QUERY,OrderTransaction.class);
                                        }
                                        ...
                                        for(OrderTransaction detail : orderTransactions) {
                                        	...
                                        	rs =  stmt.executeQuery("select firstname, lastname from employee where loginId='"+empId+"'");
                                        	if(rs.next()){
                                        ...
                                        <td><%=sr %></td>
											...
											<td><a onclick="window.open('orderFollowup?id=<%= detail.getOrderId()%>&status=<%=detail.getOrderStatus() %>&empID=<%=detail.getEmp_id() %>', '_blank', 'height=500, width=950, left=150, top=150,  titlebar=no, toolbar=no, addressbar=no, menubar=no, location=no, resizable=no');" href="#"><%=detail.getOrderStatus() %></a></td>
											<td><%=payment_status %></td>
											
											
											<td class="center">
                                                    <%if(detail.getOrderStatus().equalsIgnoreCase("Order Confirmed")){ %>  
