...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
			...
			List<Product> productList = dbAction.getDataList(QueryConstants.PRODUCT_APPROVAL_DATE_QUERY, Product.class);
				                ...
				                int sr=0;
                                       for(Product details : productList){
                                        ...
                                        <td><%=sr %></td>
												...
												<td><%=CateName %></td>
												<td><%=details.getProductBrand()%>|<%=details.getPacking()%></td>
												<td><%=details.getVegProduct()%></td>
												<td><b><%=daysBetween %></b> days</td>
												<td><a onclick="window.open('rateForm?productId=<%=details.getProduct_id() %>', '_blank', 'height=400, width=500, left=150, top=150,  titlebar=no, toolbar=no, addressbar=no, menubar=no, location=no, resizable=no');" href="#"><%if(productOtherDetail != null) { %> <%=productOtherDetail.getRateInRupee() %> <%} else{%>0<%}%></a></td>
												...
												<td>
													<%
														if(details.getStatus().equalsIgnoreCase("Y")){
															%>
																<a href="activate?id=<%= details.getProduct_id()%>&activate=N&type=product">Deactivate</a>
															<%	
														}
														else{
															%>
																<a href="activate?id=<%= details.getProduct_id()%>&activate=Y&type=product">Activate</a>
															<%
														}
													%>
												</td>
												 ...
												 <td>
													...
													<a class="glyphicon glyphicon-trash" href="delete?id=<%= details.getProduct_id()%>&type=product"></a>
												</td>
                                            </tr>
                                        <%
                                            }
